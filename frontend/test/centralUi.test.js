...
describe("visão geral, automações, histórico, destinatários e configurações", () => {
  ...
  test("configurações: funcional visível; diagnóstico técnico RECOLHIDO por padrão (sem 'open'); sem segredo", () => {
    ...
    assert.match(htmlConfiguracoes({ config: { janelaComercial: { seg_sex: { inicio: "08:00", fim: "18:00" } }, disponibilidadeIfood: { dadosDisponiveisApos: "10:00", enviosPermitidosApos: "10:30" } }, diagnostico: undefined, erros: [] }), /Lembretes do D-1 do iFood[^]*A partir das 10:30[^]*a partir das 10:00/);
