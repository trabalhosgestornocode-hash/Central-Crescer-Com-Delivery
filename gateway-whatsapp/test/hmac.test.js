// Testes do HMAC do Gateway — mesmo roteiro de cobertura do lado
// worker-martinbrower (replay, assinatura inválida, corpo adulterado,
// timestamp fora da janela), aplicado ao formato canônico deste processo.
import { test, describe, beforeEach, mock } from "node:test";
import assert from "node:assert/strict";
import {
  assinar, assinarRequisicao, exigirHmac, montarMensagem,
  _resetarNonces, _tamanhoCacheNonces, JANELA_MS,
} from "../src/hmac.js";

const SEGREDO = "s".repeat(32);

function reqFalso({ headers = {}, method = "POST", originalUrl = "/internal/comunicacao/eventos/heartbeat", body = Buffer.from("") }) {
  return {
    get: (nome) => headers[nome],
    method,
    originalUrl,
    body,
  };
}

function resFalso() {
  const res = { statusCode: null, corpo: null };
  res.status = (c) => { res.statusCode = c; return res; };
  res.json = (c) => { res.corpo = c; return res; };
  return res;
}

describe("hmac — canonical string e assinatura", () => {
  test("montarMensagem gera timestamp\\nnonce\\nMETODO\\ncaminho\\nsha256(corpo)", () => {
    const msg = montarMensagem({ timestamp: "1000", nonce: "abc", metodo: "post", caminho: "/x?y=1", corpo: "{}" });
    const linhas = msg.split("\n");
    assert.equal(linhas.length, 5);
    assert.equal(linhas[0], "1000");
    assert.equal(linhas[1], "abc");
    assert.equal(linhas[2], "POST"); // método sempre maiúsculo
    assert.equal(linhas[3], "/x?y=1");
    assert.equal(linhas[4].length, 64); // sha256 hex
  });

  test("assinarRequisicao produz headers consumíveis por exigirHmac", () => {
    const corpo = JSON.stringify({ a: 1 });
    const headers = assinarRequisicao({ segredo: SEGREDO, metodo: "POST", caminho: "/internal/comunicacao/eventos/heartbeat", corpo });
    assert.ok(headers["X-Gateway-Timestamp"]);
    assert.ok(headers["X-Gateway-Nonce"]);
    assert.ok(headers["X-Gateway-Signature"]);
  });
});

describe("hmac — middleware exigirHmac", () => {
  beforeEach(() => _resetarNonces());

  function chamada({ segredo = SEGREDO, metodo = "POST", caminho = "/internal/comunicacao/eventos/heartbeat", corpo = "{}" } = {}) {
    const headers = assinarRequisicao({ segredo, metodo, caminho, corpo });
    return { headers, corpo };
  }

  test("aceita uma requisição corretamente assinada", () => {
    const { headers, corpo } = chamada();
    const req = reqFalso({ headers, method: "POST", originalUrl: "/internal/comunicacao/eventos/heartbeat", body: Buffer.from(corpo) });
    const res = resFalso();
    const next = mock.fn();
    exigirHmac(SEGREDO)(req, res, next);
    assert.equal(next.mock.calls.length, 1);
    assert.deepEqual(req.corpoJson, { a: 1 } === undefined ? {} : JSON.parse(corpo));
  });

  test("rejeita cabeçalho ausente", () => {
    const req = reqFalso({ headers: {} });
    const res = resFalso();
    exigirHmac(SEGREDO)(req, res, mock.fn());
    assert.equal(res.statusCode, 401);
  });

  test("rejeita assinatura inválida", () => {
    const { corpo } = chamada();
    const req = reqFalso({
      headers: { "X-Gateway-Timestamp": String(Date.now()), "X-Gateway-Nonce": "a".repeat(24), "X-Gateway-Signature": "0".repeat(64) },
      body: Buffer.from(corpo),
    });
    const res = resFalso();
    exigirHmac(SEGREDO)(req, res, mock.fn());
    assert.equal(res.statusCode, 401);
  });

  test("rejeita corpo adulterado após a assinatura", () => {
    const { headers } = chamada({ corpo: JSON.stringify({ a: 1 }) });
    const req = reqFalso({ headers, body: Buffer.from(JSON.stringify({ a: 2 })) });
    const res = resFalso();
    exigirHmac(SEGREDO)(req, res, mock.fn());
    assert.equal(res.statusCode, 401);
  });

  test("rejeita path alterado após a assinatura", () => {
    const { headers, corpo } = chamada({ caminho: "/internal/comunicacao/eventos/heartbeat" });
    const req = reqFalso({ headers, originalUrl: "/internal/comunicacao/eventos/mensagem-recebida", body: Buffer.from(corpo) });
    const res = resFalso();
    exigirHmac(SEGREDO)(req, res, mock.fn());
    assert.equal(res.statusCode, 401);
  });

  test("rejeita método alterado após a assinatura", () => {
    const { headers, corpo } = chamada({ metodo: "POST" });
    const req = reqFalso({ headers, method: "GET", body: Buffer.from(corpo) });
    const res = resFalso();
    exigirHmac(SEGREDO)(req, res, mock.fn());
    assert.equal(res.statusCode, 401);
  });

  test("rejeita nonce reutilizado (replay)", () => {
    const { headers, corpo } = chamada();
    const req1 = reqFalso({ headers, body: Buffer.from(corpo) });
    exigirHmac(SEGREDO)(req1, resFalso(), mock.fn());

    const req2 = reqFalso({ headers, body: Buffer.from(corpo) });
    const res2 = resFalso();
    exigirHmac(SEGREDO)(req2, res2, mock.fn());
    assert.equal(res2.statusCode, 401);
  });

  test("rejeita timestamp fora da janela (passado)", () => {
    const timestamp = String(Date.now() - JANELA_MS - 5000);
    const nonce = "b".repeat(24);
    const corpo = "{}";
    const assinatura = assinar({ segredo: SEGREDO, timestamp, nonce, metodo: "POST", caminho: "/x", corpo });
    const req = reqFalso({
      headers: { "X-Gateway-Timestamp": timestamp, "X-Gateway-Nonce": nonce, "X-Gateway-Signature": assinatura },
      originalUrl: "/x", body: Buffer.from(corpo),
    });
    const res = resFalso();
    exigirHmac(SEGREDO)(req, res, mock.fn());
    assert.equal(res.statusCode, 401);
  });

  test("rejeita timestamp excessivamente futuro", () => {
    const timestamp = String(Date.now() + JANELA_MS + 5000);
    const nonce = "c".repeat(24);
    const corpo = "{}";
    const assinatura = assinar({ segredo: SEGREDO, timestamp, nonce, metodo: "POST", caminho: "/x", corpo });
    const req = reqFalso({
      headers: { "X-Gateway-Timestamp": timestamp, "X-Gateway-Nonce": nonce, "X-Gateway-Signature": assinatura },
      originalUrl: "/x", body: Buffer.from(corpo),
    });
    const res = resFalso();
    exigirHmac(SEGREDO)(req, res, mock.fn());
    assert.equal(res.statusCode, 401);
  });

  test("rejeita nonce malformado", () => {
    const req = reqFalso({
      headers: { "X-Gateway-Timestamp": String(Date.now()), "X-Gateway-Nonce": "**", "X-Gateway-Signature": "0".repeat(64) },
    });
    const res = resFalso();
    exigirHmac(SEGREDO)(req, res, mock.fn());
    assert.equal(res.statusCode, 401);
  });

  test("limpa nonces do cache (não cresce indefinidamente)", () => {
    for (let i = 0; i < 5; i++) {
      const { headers, corpo } = chamada({ corpo: JSON.stringify({ i }) });
      exigirHmac(SEGREDO)(reqFalso({ headers, body: Buffer.from(corpo) }), resFalso(), mock.fn());
    }
    assert.equal(_tamanhoCacheNonces(), 5);
  });

  test("exigirHmac lança na criação se o segredo estiver ausente (fail-closed)", () => {
    assert.throws(() => exigirHmac(undefined), /WHATSAPP_GATEWAY_SECRET ausente/);
  });
});
