...
const CHAVES = Object.freeze(["contratoStatus", "providerMessageId", "status", "ocorridoEm", "ackTipo", "erroCodigo"]);
 ...
 * @returns {{ok: true, evento: {providerMessageId: string, status: string, ocorridoEm: string|null, ackTipo: string|null, erroCodigo: string|null}} | {ok: false, erro: string}}
...
export function validarEventoStatusProvider(corpo, { agora = () => Date.now() } = {}) {
  ...
  const c = corpo;
  if (c.contratoStatus !== CONTRATO_STATUS_VERSAO) return { ok: false, erro: "contratoStatus" };
  ...
  return { ok: true, evento: { providerMessageId: c.providerMessageId, status: c.status, ocorridoEm, ackTipo, erroCodigo } };
}
